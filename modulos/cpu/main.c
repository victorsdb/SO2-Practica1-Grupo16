...
int escribir_archivo(struct seq_file *archivo, void *v)
{
	...
	seq_printf(archivo, "{");
    seq_printf(archivo, "\"porcentaje\": \"%li\"", usage_cpu * 10000 / total_cpu  );
	seq_printf(archivo, "}");
	return 0;
}
...
int  iniciar(void) 
{
    ...
    entry = proc_create("cpu_grupo16", 0, NULL, &operaciones);
    
    if(!entry) 
        return -1; 
    else 
        printk(KERN_INFO "Hola mundo, somos el grupo 16 y este es el monitor de cpu.\n");
...
void salir(void)
{
    ...
    printk(KERN_INFO "Sayonara mundo, somos el grupo 16 y este fue el monitor de cpu.\n");
}
